...
export const ResultDisplay: React.FC<ResultDisplayProps> = ({
  ...
  return (
    <div className="h-full bg-[#f8fafc] rounded-3xl shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] border border-gray-100 flex flex-col relative overflow-hidden">
        ...
        <div className="py-10 px-2 flex justify-center items-start bg-[#f8fafc]">
            ...
            <div className="flex flex-col items-center gap-6 animate-fade-in" style={{ animationDelay: '0.2s' }}>
                {resultMockupData ? (
                    ...
                    {/* Action Toolbar */}
                    <div className="flex items-center gap-3 bg-white border border-gray-200 shadow-lg shadow-gray-200/50 rounded-2xl p-2 transform transition-all hover:scale-105 mb-24">
                        <button 
                            onClick={handleCopy}
                            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold transition-all ${copied ? 'bg-green-50 text-green-600' : 'hover:bg-gray-50 text-gray-700'}`}
                        >
                        ...
                        {activeVariant?.enhanced_images_base64 && activeVariant.enhanced_images_base64.length > 0 && (
                            <>
                            <div className="w-px h-5 bg-gray-200"></div>
                            <button 
                                onClick={handleDownloadImage}
                                className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold hover:bg-gray-50 text-gray-700 transition-all"
                            >
      ...
      {/* 3. FIXED BOTTOM: Refine Chat Bar */}
      {resultMockupData && (
        <div className="flex-none bg-white border-t border-gray-100 p-4 pb-6 z-20 shadow-[0_-4px_20px_-10px_rgba(0,0,0,0.05)]">
            <div className="max-w-2xl mx-auto w-full flex flex-col gap-3">
                ...
                <div className="relative w-full">
                    <input 
                        ...
                        className="w-full bg-gray-50 border border-gray-200 rounded-2xl pl-4 pr-12 py-3.5 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-purple-100 focus:border-purple-300 transition-all shadow-inner"
                    />
                    <button 
                        onClick={() => triggerRefine('none')}
                        disabled={!refineInput.trim()}
                        className={`absolute right-2 top-2 w-9 h-9 flex items-center justify-center rounded-xl transition-all ${
                            refineInput.trim() 
